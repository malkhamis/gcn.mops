...
cpu.time.chkX <<- as.numeric(difftime(end.time.chkX ,start.time.chkX, unit = "secs"))
...
cpu.time.formChkInput <<- as.numeric(difftime(end.time ,start.time, unit = "secs"))
...
cpu.time.normalization <<- as.numeric(difftime(end.time ,start.time, unit = "secs"))
...
cpu.time.modeling <<- as.numeric(difftime(end.time, start.time, unit="secs"))
...
start.time <- Sys.time()
	...
	L <- sapply(res,.subset2,1)
	L <- t(L)
	rownames(L) <- rownames(X)
	colnames(L) <- classes
	params$L <- L
	
	A <- sapply(res,.subset2,2)
	A <- t(A)
	rownames(A) <- rownames(X)
	colnames(A) <- classes
	params$A <- A

	CN <- sapply(res,.subset2,3)
	CN <- t(CN)
	rownames(CN) <- rownames(X)
	colnames(CN) <- colnames(X)
	
	sINI <- sapply(res,.subset2,4)	
	sINI <- t(sINI)
...
cpu.time.postprocessing <<- as.numeric(difftime(end.time, start.time, unit="secs"))
...
cpu.time.seg <<- as.numeric(difftime(end.time.sg, start.time.sg, unit="secs"))
...
cpu.time.cnvCalling <<- as.numeric(difftime(end.time, start.time, unit="secs"))
